...
library(tidyverse)
library(lubridate)
source("~/projects/mscthesis/scripts/final/data_extraction_optimal_temperature_fit.R")
...
- Data extraction followed the filtering criteria applied by K19.
- Only subset for mature native environment and seasonal data were taken.
- Running all chunks in this rmarkdown extracts all datapoints for optimal temperature used in this thesis.
- Subchapter names refer to the first author's last name. See metainfo file loaded at bottom of this script for more information.
...
df_togashi <- read_csv(paste(path,"/aci_datapoints/gwwACidata_processed_new.csv", sep="")) %>% 
  ...
  mutate(dataset = "togashi",
         Season = ifelse(Season == "Summer", "2017-06-21", Season),  # Assumed winter and summer mid in southern hemisphere, togashi uses "2013-04-04" for all fits to get Tavg_30
...
### Alida
```{r}
df_alida <- read_csv(paste0(path,"/aci_fits/alida_mao.csv")) %>%
  mutate(dataset = "alida",
        sample_date = dmy(date),
        agg_date = round_date(sample_date, unit = "week")) %>%
  group_by(dataset,
...
df_alida %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, agg_date, pvalue, topt)
```

### Anna
```{r}
df_anna <- read_csv(paste0(path,"/aci_fits/Anna.csv")) %>%
  mutate(dataset = "anna",
         sample_date = as_date(DOY-1, origin = paste0(Year, "-01-01")),  # Converting DOY and year information into day, -1 needed because as_date takes 0 as 1. January)
         agg_date = sample_date) %>%
  group_by(dataset,
           agg_date) %>%
  nest() %>% 
  mutate(fit_opt = purrr::map(data, ~fit_nonlinear_topt(dat = .x, x = x, y = y, random = "Cohort_age")))
...
### Rogers
```{r}
df_arctic <- read_csv(paste0(path,"/aci_fits/Rogers.csv")) %>%
  dplyr::filter(maxCi > 400) %>% 
  mutate(dataset = "arctic",
         sample_date = ymd(Sample_Date),
         agg_date = floor_date(sample_date, unit = "week")) %>% 
  group_by(dataset,
           agg_date) %>% 
  nest() %>% 
  mutate(fit_opt = purrr::map(data, ~fit_nonlinear_topt(dat = .x, x = x, y = y, random = "USDA_Species_Code")))

df_arctic %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, fit_method, pvalue, b, aopt, topt, agg_date)
```

### Ellsworth
```{r}
df_ellsworth <- read_csv(paste0(path,"/aci_fits/Ellsworth.csv")) %>% 
  mutate(dataset = "ellsworth",
         sample_date = dmy(paste("15-", Date)),
         agg_date = sample_date) %>% 
  group_by(dataset,
...
df_ellsworth %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, fit_method, b, aopt, topt, agg_date)
```

### UWS (eucface)
```{r}
df_eucface <- read_csv(paste0(path,"/aci_fits/eucface.csv")) %>%
  mutate(dataset = "eucface",
         sample_date = NA,
         agg_date = sample_date) %>% 
...
df_eucface %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue)
```

### Han (cypress)
```{r}
df_han_cypress <- read_csv(paste0(path,"/aci_fits/han_a.csv")) %>%
  mutate(dataset = "han_cypress",
         sample_date = my(paste0(Season, "2004")),
         agg_date = sample_date) %>% 
  group_by(dataset,
...
### Han (pine)
```{r}
df_han_pine <- read_csv(paste0(path,"/aci_fits/han_etal.csv")) %>%
  #dplyr::filter(!(Curve %in% c(5, 9))) %>% 
  mutate(dataset = "han_pine",
         sample_date = ifelse(Season == "May",paste0("15-", Season, "-2002"), paste0("15-", Season, "-2001")),
         sample_date = dmy(sample_date),
         agg_date = sample_date) %>% 
...
df_han_pine %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue)
```

### Hikosaka
```{r}
df_hikosaka <- read_csv(paste0(path,"/aci_fits/hikosaka.csv")) %>%
  mutate(dataset = "hikosaka",
         Date = ifelse(Date == "2001-06-01", "2001-06-11", Date), 
         Date = ifelse(Date == "2001-08-01", "2001-08-10", Date), 
         Date = ifelse(Date == "2001-09-01", "2001-09-20", Date), 
...
df_hikosaka %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, fit_method, b, aopt, topt, agg_date, pvalue)
```

### Kelly
```{r}
df_kelly <- read_csv(paste0(path,"/aci_fits/kelly.csv")) %>%
  mutate(dataset = "kelly",
         Season = ifelse(Season == "SUMMER", "2011-04-07", Season), 
         Season = ifelse(Season == "WINTER", "2010-07-22", Season),  
         sample_date = ymd(Season),
...
df_kelly %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue)
```

### Carter
```{r}
df_kelsey <- read_csv(paste0(path,"/aci_fits/kelsey_cater.csv")) %>%
  dplyr::filter(Plant_functional_type != "shrub") %>% 
  mutate(dataset = "kelsey",
         sample_date = dmy(Date),
         agg_date = sample_date) %>%
  group_by(dataset,
...
### Medly (eucalypt)
```{r}
df_medlyn_eucalypt <- read_csv(paste0(path,"/aci_fits/tumbarumba.csv")) %>% 
  mutate(dataset = "medlyn_eucalypt",
         Leaf.Age = as.factor(Leaf.Age),
         sample_date = ymd(Date),
         agg_date = round_date(sample_date, unit = "week")) %>% 
  group_by(dataset,
           agg_date) %>%
  nest() %>% 
	mutate(fit_opt = purrr::map(data, ~fit_nonlinear_topt(dat = .x, x = x, y = y, random = "Leaf.Age")))

df_medlyn_eucalypt %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue)
```

### Medlyn (pine)
```{r}
df_medlyn_pine <- read_csv(paste0(path,"/aci_fits/medlyn.csv")) %>%
  dplyr::filter(!(Species == "Tamjoute")) %>%
  mutate(dataset = "medlyn_pine",
         Species = ifelse(Species == "Landes", "Pinus pinaster_L", Species),
         Season = ifelse(Season == "Jul", paste(mean.Date(c(dmy("29/6/1999"), dmy("8/7/1999")))), Season),
         Season = ifelse(Season == "AugSept", paste(mean.Date(c(dmy("24/8/1999"), dmy("1/9/1999")))), Season),
...
df_medlyn_pine %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue, fit_method)
```

### Onoda
```{r}
df_onoda <- read_csv(paste0(path,"/aci_fits/onoda.csv")) %>%
  dplyr::filter(GrowthCa == "amb",
         lowAs < 9.5) %>% 
  mutate(dataset = "onoda",
         Season = ifelse(Season == "May", paste(mean.Date(c(dmy("08-05-2002"), dmy("12-05-2002")))),  Season),
...
### Savanna
```{r}
df_sav <- read_csv(paste0(path,"/aci_fits/cernussac.csv")) %>% 
  dplyr::filter(Canopy_position == "Canopy") %>% 
  mutate(dataset = "savanna",
         sample_date = dmy(Date),
         agg_date = sample_date) %>% 
  group_by(dataset,
...
df_sav %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue)
```

### Slot
```{r}
df_slot <- read_csv(paste0(path,"/aci_fits/martijn_slot.csv")) %>%
  dplyr::filter(!(Curve %in% c(133, 135))) %>% 
  mutate(dataset = "slot",
         sample_date = dmy(Date),
         agg_date = round_date(sample_date, unit = "week")) %>% 
  group_by(dataset,
...
df_slot %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue, r2)
```

### Tarvainen (pine)
```{r}
df_tarvainen_pine <- read_csv(paste0(path,"/aci_fits/scots_pine_tarvainen.csv")) %>%
  mutate(dataset = "tarvainen_pine",
         sample_date = dmy(paste0(Date, "2013")),
         agg_date = round_date(sample_date, unit = "week")) %>% 
  group_by(dataset,
...
df_tarvainen_pine %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue, r2)
```

### Tarvainen (spruce)
```{r}
df_tarvainen_spruce <- read_csv(paste0(path,"/aci_fits/tarvainen_etal.csv")) %>%
  mutate(dataset = "tarvainen_spruce",
         Season = ifelse(Season == "Year1", "2009-06-15", Season),
         Season = ifelse(Season == "Year3", "2009-09-15", Season),
         sample_date = ymd(Season),
...
df_tarvainen_spruce %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, fit_method, b, aopt, topt)
```

### Togashi
```{r}
df_togashi <- read_csv(paste0(path,"/aci_fits/togashi.csv")) %>%
  #dplyr::filter(!(Curve %in% c(42, 43, 41, 45, 44))) %>% 
  mutate(dataset = "togashi",
         Season = ifelse(Season == "Summer", "2017-06-21", Season),  # Assumed winter and summer mid in southern hemisphere
         Season = ifelse(Season == "Winter", "2017-12-21", Season),  # Year of publication assumed, southern hemisphere mid-season assumed
         sample_date = ymd(Season),
         agg_date = sample_date) %>% 
  group_by(dataset,
...
# Species as aggregation level to check goodness of fits
# have to check if I can infer better dates of measurements instead of winter and summer...

df_togashi %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue) %>% arrange(pvalue)
```

### Tribuzy
```{r}
df_tribuzi <- read_csv(paste0(path,"/aci_fits/tribuzi.csv")) %>%
  mutate(dataset = "tribuzi",
         Season = ifelse(Season == "wet", "2005-04-15", Season), # wet/dry but no actual information found
         Season = ifelse(Season == "dry", "2005-10-15", Season),
         sample_date = ymd(Season), 
...
df_tribuzi %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue)
```

### Wang
```{r}
df_wang <- read_csv(paste0(path,"/aci_fits/wang et al.csv")) %>%
  dplyr::filter(!(Species == "Betula pendula")) %>%
  mutate(dataset = "wang",
         sample_date = dmy(ifelse(Date == "Jul-98", "15-07-1994", Date)),
         agg_date = sample_date) %>% 
  group_by(dataset,
...
df_wang %>% unnest(cols = "fit_opt") %>% dplyr::select(dataset, data, agg_date, pvalue, r2)
```
...
df_wtc <- bind_rows(read_csv(paste0(path,"/aci_fits/wtc1.csv")) %>% 
                    ...
                    read_csv(paste0(path,"/aci_fits/wtc2.csv")) %>%
                       mutate(dataset = "wtc2",
                              Date = ymd(Date)) %>%  
                       dplyr::filter(Temp.treat == "ambient"),
  ...
  mutate(dataset = "wtc",
         sample_date = Date,
         # agg_date = round_date(sample_date, unit = "month")) %>%
  			 agg_date = round_date(sample_date, unit = "week")) %>%
  group_by(dataset,
           agg_date) %>%
  nest() %>% 
...
saveRDS(df_275_opt, "~/data/mscthesis/raw/photom_kumarathunge2019/df_275_opt_preQC.rds")
```
...
pval <- 0.1
all  <- df_275_opt %>% nrow
sign <- df_275_opt %>% unnest(cols = "fit_opt") %>% dplyr::filter(pvalue < pval) %>% nrow()
cat("\n p-values below", pval*100,"%: ", sign, "/", all, "| which is", round(sign/all, 2)*100, "% \n")

cat("\n Free Ci: --------------------------------------")
pval <- 0.1
all  <- df_obs_opt %>% nrow
sign <- df_obs_opt %>% unnest(cols = "fit_opt") %>% dplyr::filter(pvalue < pval) %>% nrow()
cat("\n p-values below", pval*100,"%: ", sign, "/", all, "| which is", round(sign/all, 2)*100, "% \n")
```
...
for (subset in c("obs", "275")) {
	...
	df_1 <- readRDS(paste0("~/data/mscthesis/raw/photom_kumarathunge2019/df_", subset, "_opt_preQC.rds"))
...
##      - Ecophysiologically impossible optmal temperature
##
...
# Fixed Ci conditions
df_275_qc <- readRDS("~/data/mscthesis/raw/photom_kumarathunge2019/df_275_opt_preQC.rds") %>% 
	dplyr::filter(!(dataset == "alida" & agg_date == "2014-08-24"), # (1)
								!(dataset == "anna" & agg_date == "2011-07-14"), # (1)
								!(dataset == "anna" & agg_date == "2011-10-20"), # (1)
								!(dataset == "anna" & agg_date == "2013-04-23"), # (2)
								!(dataset == "anna" & agg_date == "2013-04-24"), # (2)
								!(dataset == "arctic" & agg_date == "2012-08-05"), # (1)
								!(dataset == "ellsworth" & agg_date == "1998-12-15"), # (2)
								!(dataset == "han_pine" & agg_date == "2001-11-15"), # (1)
								!(dataset == "hikosaka" & agg_date == "2001-08-01"), # (1)
								!(dataset == "kelsey" & agg_date == "2015-07-04"), # (1)
								!(dataset == "kelsey" & agg_date == "2015-07-06"), # (1)
								!(dataset == "kelsey" & agg_date == "2015-07-11"), # (2)
								!(dataset == "kelsey" & agg_date == "2015-07-12"), # (2)
								!(dataset == "kelsey" & agg_date == "2015-07-20"), # (1)
								!(dataset == "kelsey" & agg_date == "2015-07-21"), # (2)
								!(dataset == "medlyn_eucalypt" & agg_date == "2001-11-18"), # (2)
								!(dataset == "medlyn_eucalypt" & agg_date == "2002-02-17"), # (2) # TODO: Could be revised by extracting fit-defining outlier
								!(dataset == "medlyn_eucalypt" & agg_date == "2002-05-05"), # (1)
								!(dataset == "medlyn_pine" & agg_date == "1999-07-03"), # (2)
								!(dataset == "medlyn_pine" & agg_date == "1999-11-20"), # (1)
								!(dataset == "medlyn_pine" & agg_date == "2000-01-28"), # (1)
								!(dataset == "onoda" & agg_date == "2002-08-02"), # (1)
								!(dataset == "slot" & agg_date == "2016-11-27"), # (1)
								!(dataset == "slot" & agg_date == "2016-12-11"), # (1)
								!(dataset == "slot" & agg_date == "2016-12-18"), # (2)
								!(dataset == "slot" & agg_date == "2016-12-25"), # (2)
								!(dataset == "togashi" & agg_date == "2017-06-21"), # (2)
								!(dataset == "togashi" & agg_date == "2017-12-21"), # (2)
								!(dataset == "tribuzi" & agg_date == "2005-10-15"), # (1)
								!(dataset == "wtc" & agg_date == "2011-09-11") # (1)
								)
...
```{r}
cat("\n QC: --------------------------------------------",
		"\n Fits removed from fixed Ci dataset:", nrow(df_275_opt) - nrow(df_275_qc),
		"\n Fits removed from free  Ci dataset:", nrow(df_obs_opt) - nrow(df_obs_qc), "\n")

cat("\n Fixed Ci: --------------------------------------")
pval <- 0.1
all  <- df_275_qc %>% nrow
sign <- df_275_qc %>% unnest(cols = "fit_opt") %>% dplyr::filter(pvalue < pval) %>% nrow()
cat("\n p-values below", pval*100,"%: ", sign, "/", all, "| which is", round(sign/all, 2)*100, "% \n")
