...
calc_optimal_tcleaf_vcmax_jmax <- function(tc_leaf = 25,
    ...
    optimise_this_tcleaf_vcmax_jmax <- function( par, args, iabs, kphio, beta, method_jmaxlim_inst, maximize=FALSE, return_all=FALSE ){
        ...
        kphio     <- kphio * calc_ftemp_kphio( tc_leaf )
        ...
        if (method_jmaxlim_inst == "smith37") {
            ...
            c_cost <- 0.103 # Estimated by Wang et al. (2017)
        
            } else if (method_jmaxlim_inst == "farquhar89") {
            ...
            c_cost <- 0.053 # Estimated by Smith et al. (2019)
        }
    ...
    out_optim <- optimr::optimr(
        ...
        iabs       = (ppfd * fapar), # Input for optimr must be in mol/m2/d to yield sensible optimization
        kphio      = kphio,
        beta       = beta,
        method_jmaxlim_inst = method_jmaxlim_inst,
        method     = "L-BFGS-B",
...
LeafEnergyBalance <- function(Tleaf = 21.5,  # Input in degC
    ...
    gs   <- gs * 10^-6 * Patm       # mol/m2/s/Pa to ppm mol/m2/s
...
calc_tc_leaf_from_tc_air <- function(tc_air   = 25,     # input in degC
    ...
    sol_optimr <-	optimr::optimr(
        ...
        gs        = gs,        # input in mol/m2/s/Pa
        VPD       = vpd,       # input in Pa
        Patm      = patm,      # input in Pa
        PPFD      = ppfd,      # input in mol/m2/s
...
maximize_this_tc_leaf <- function(tc_leaf   = 25, # This gets optimized in optimr()
    ...
    if (method_eb == "plantecophys") {
        ...
        # Get relative humidity from vapor pressure deficit: vpd = e_sat * (100 - RH)/100
        RH <- (100 - ((vpd * 100) / e_sat(tc_air, patm))) / 100
    ...
    sol_optimize <- tryCatch(
        {
            sol_optimize <- optimize(maximize_this_tc_leaf,
                                     interval  = c(tc_air - 15, tc_air + 15),
                                     tc_air    = tc_air,
                                     tc_growth = tc_growth,
                                     tc_home   = tc_home,
                                     ppfd      = ppfd,
